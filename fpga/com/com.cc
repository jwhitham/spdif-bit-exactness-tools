...
size_t copySamples(BYTE* pData, const int16_t* sampleData, const uint32_t sampleIndex,
                const uint32_t sampleCount, UINT32 bufferFrameCount, WORD nChannels)
{
    ...
    memcpy(pData, &sampleData[(size_t) sampleIndex * (size_t) nChannels], (size_t) samplesToCopy * sizeof(int16_t) * (size_t) nChannels);
...
HRESULT com_send(const uint32_t numPackets, const uint64_t* packetData)
{
    ...
    EXIT_ON_ERROR(hr)
    printf("%d %d\n", (int) pwfx->nSamplesPerSec, (int) pwfx->nChannels);
